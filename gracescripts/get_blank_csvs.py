...
event_df.to_csv('TEMPLATE_EventData.csv',index=False)

#for table DNAExtractions
event_df = pd.DataFrame(columns=dna_cols)
event_df.to_csv('TEMPLATE_DNAExtractions.csv',index=False)

#for table SpecimenData
event_df = pd.DataFrame(columns=spec_cols)
event_df.to_csv('TEMPLATE_SpecimenData.csv',index=False)

#for table GenomicLibraries
event_df = pd.DataFrame(columns=genomlib_cols)
event_df.to_csv('template/CSVsTEMPLATE_GenomicLibraries.csv',index=False)
